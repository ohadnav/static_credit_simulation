...
from common.local_numbers import Float, Dollar, O, ONE, TWO, Duration, O_INT, Ratio, Int, Date, ONE_INT, Percent
...
from loan_simulation_results import O_LSR
...
class TestLoanSimulation(BaseTestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.loan_simulation.last_year_revenue, O)
        self.assertEqual(self.loan_simulation.ledger.num_loans(), O_INT)
    ...
    def test_add_debt(self):
        ...
        self.assertEqual(self.loan_simulation.ledger.num_loans(), 1)
        ...
        self.assertEqual(self.loan_simulation.ledger.num_loans(), 2)
    ...
    def test_loan_amount(self):
        self.assertEqual(
            self.loan_simulation.loan_amount(), self.loan_simulation.merchant.annual_top_line(
                self.data_generator.start_date) * self.context.loan_amount_per_monthly_income / constants.NUM_MONTHS)
        self.loan_simulation.today += self.context.history_duration_for_amount_calculation
        self.loan_simulation.recent_history_revenue = self.context.history_duration_for_amount_calculation / \
                                                      constants.MONTH
        self.assertEqual(self.loan_simulation.loan_amount(), self.context.loan_amount_per_monthly_income)
        self.loan_simulation.recent_history_revenue = Dollar(10 ** 10)
        self.assertEqual(self.loan_simulation.loan_amount(), self.context.max_loan_amount)
    ...
    def test_primary_approval_conditions(self):
        ...
        self.loan_simulation.credit_needed = MagicMock(return_value=self.context.min_loan_amount)
    ...
    def test_secondary_approval_conditions(self):
        ...
        self.assertFalse(self.loan_simulation.secondary_approval_conditions())
        ...
        self.assertTrue(self.loan_simulation.secondary_approval_conditions())
    ...
    def test_remaining_credit(self):
        ...
        self.assertEqual(self.loan_simulation.remaining_credit(), self.loan_simulation.loan_amount())
        self.loan_simulation.add_debt(ONE)
        self.assertEqual(self.loan_simulation.remaining_credit(), self.loan_simulation.loan_amount() - 1)
    ...
    def test_simulate_sales_recent_revenue(self):
        self.merchant.revenue_per_day = MagicMock(return_value=ONE)
        self.loan_simulation.simulate_sales()
        self.assertEqual(self.loan_simulation.recent_history_revenue, ONE)
        self.loan_simulation.today += self.context.history_duration_for_amount_calculation
        self.loan_simulation.simulate_sales()
        self.assertEqual(self.loan_simulation.recent_history_revenue, ONE)

    def test_simulate_sales_last_year_revenue(self):
        self.merchant.revenue_per_day = MagicMock(return_value=ONE)
        self.loan_simulation.simulate_sales()
        self.assertEqual(
            self.loan_simulation.marketplace_balance, self.merchant.gp_per_day(self.data_generator.start_date))
        self.assertEqual(self.loan_simulation.last_year_revenue, ONE)
    ...
    def test_calculate_results(self):
        rand_results = {field.name: self.data_generator.random() for field in fields(LoanSimulationResults)}
        rand_results['num_loans'] = Int(randint(1, 1000))
        for obj in [self.loan_simulation, self.loan_simulation.ledger, self.merchant]:
            for k, v in rand_results.items():
                if hasattr(obj, k):
                    setattr(obj, k, MagicMock(return_value=v))
        self.loan_simulation.calculate_results()
        self.assertEqual(self.loan_simulation.simulation_results, dacite.from_dict(LoanSimulationResults, rand_results))

    def test_calculate_bankruptcy_rate(self):
        self.loan_simulation.bankruptcy_date = None
        self.assertEqual(self.loan_simulation.bankruptcy_rate(), 0)
        self.loan_simulation.bankruptcy_date = self.data_generator.simulated_duration / 10 + \
                                               self.data_generator.start_date
        self.assertEqual(self.loan_simulation.bankruptcy_rate(), 0.9)
        self.loan_simulation.bankruptcy_date = self.data_generator.start_date
        self.assertEqual(self.loan_simulation.bankruptcy_rate(), 1)
        self.loan_simulation.bankruptcy_date = self.data_generator.simulated_duration
        self.assertEqual(
            self.loan_simulation.bankruptcy_rate(),
    ...
    def test_revenue_cagr(self):
        ...
        self.loan_simulation.last_year_revenue = Dollar(3)
    ...
    def test_cost_of_capital(self):
        ...
        self.assertEqual(self.loan_simulation.cost_of_capital(), repaid1 * Float.mean(coc[:2]))
        ...
        self.loan_simulation.today += self.context.marketplace_payment_cycle
        self.assertEqual(self.loan_simulation.cost_of_capital(), repaid1 * Float.mean(coc[1:3]))
        # Second debt
    ...
    def test_average_apr(self):
        ...
        self.loan_simulation.add_debt(self.loan_simulation.debt_to_loan_amount(debt1))
        self.assertEqual(self.loan_simulation.effective_apr(), Float.mean(apr[:2]))
        self.loan_simulation.projected_remaining_debt = MagicMock(return_value=ONE)
        ...
        self.loan_simulation.projected_amount_per_repayment = MagicMock(return_value=ONE)
        self.loan_simulation.today += self.context.marketplace_payment_cycle
        self.assertEqual(self.loan_simulation.effective_apr(), Float.mean(apr[1:3]))
        self.loan_simulation.add_debt(self.loan_simulation.debt_to_loan_amount(debt2))
    ...
    def test_take_snapshot(self):
        ...
        self.loan_simulation.current_simulation_results = MagicMock(return_value=O_LSR)
        self.loan_simulation.simulate()
        expected = {Date(day): O_LSR for day in range(
            self.context.snapshot_cycle + self.data_generator.start_date - 1, self.loan_simulation.today + 1,
            self.context.snapshot_cycle)}
        # noinspection PyTypeChecker
        self.assertDeepAlmostEqual(expected, self.loan_simulation.snapshots)

    def test_estimated_annual_revenue(self):
        self.assertEqual(
            self.merchant.annual_top_line(self.data_generator.start_date),
            self.loan_simulation.estimated_annual_revenue())
        self.loan_simulation.today = Date(constants.MONTH)
        self.assertEqual(
            self.merchant.annual_top_line(self.data_generator.start_date),
            self.loan_simulation.estimated_annual_revenue())
        self.loan_simulation.today = Date(constants.YEAR / 2)
        self.loan_simulation.last_year_revenue = ONE
        self.assertEqual(self.loan_simulation.last_year_revenue * 2, self.loan_simulation.estimated_annual_revenue())
        self.loan_simulation.today = Date(constants.YEAR)
        self.assertEqual(self.loan_simulation.last_year_revenue, self.loan_simulation.estimated_annual_revenue())
        self.loan_simulation.today = Date(constants.YEAR * 2)
        self.assertEqual(self.loan_simulation.last_year_revenue, self.loan_simulation.estimated_annual_revenue())
